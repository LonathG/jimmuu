...
const handleMouseOver = (e) => {
	...
	for (
		...
		i <= Math.min(spans.length - 1, index + 5);
		i++
	) {
...
const Description = () => {
	...
	return (
		...
		>
			<motion.div
				className="large-title text-3xl md:text-4xl lg:text-5xl font-semi-bold tracking-wide mt-[3rem]"
				initial="hidden"
				animate={controls}
				variants={fadeInUp}
			...
			<div className="sub_description flex flex-col md:flex-row justify-center gap-2 md:gap-8 mb-[4rem]">
				{string.MainPoints.map((point, index) => (
					<motion.div
						...
						transition={{
							duration: 0.8,
							delay: index == 0 ? 0.8 : index == 1 ? 1.2 : 1.6,
						}} // Stagger effect
						onMouseOver={handleMouseOver}
						onMouseOut={handleMouseOut}
					>
